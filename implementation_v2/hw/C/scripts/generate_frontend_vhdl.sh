#!/bin/bash
DEFINES="-D FRONTEND -D USE_DOUBLE -D FPGA -D READBACK"
...
AaLinkExtMem .Aa/band_pass_filter.aa .Aa/best_fit.aa .Aa/beat.aa .Aa/derivative.aa .Aa/frontend.aa .Aa/hermite_fitter.aa .Aa/moving_average.aa .Aa/qrs_peak_detect.aa Aa/utils.aa | vcFormat > .Aa/ecg.linked.aa
AaOpt -C .Aa/ecg.linked.aa | vcFormat > .Aa/ecg.linked.optC.aa
AaOpt -B .Aa/ecg.linked.optC.aa | vcFormat > .Aa/ecg.linked.opt.aa
Aa2VC -O -C -r controllerDaemon -r hermiteFitterDaemon .Aa/ecg.linked.opt.aa | vcFormat > .vC/ecg.vc

	
# generate vhdl from vc
rm -rf frontend_vhdl
mkdir frontend_vhdl
vc2vhdl  -O -I 2  -a -C -e ecg_frontend -w -s ghdl $TOPMODULES -f .vC/ecg.vc 
vhdlFormat < ecg_frontend_global_package.unformatted_vhdl > frontend_vhdl/ecg_frontend_global_package.vhdl
vhdlFormat < ecg_frontend.unformatted_vhdl > frontend_vhdl/ecg_frontend.vhdl
vhdlFormat < ecg_frontend_test_bench.unformatted_vhdl > frontend_vhdl/ecg_frontend_test_bench.vhdl
