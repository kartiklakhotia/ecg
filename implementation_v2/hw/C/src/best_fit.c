...
void  __InnerProduct__(int I) {
	...
	for(SI = 0; SI < NSIGMAS; SI++)
	{
		for(H = 0; H < NHERMITE; H++)
		{
			...
			__dotp_mem__(H,SI) = fp64_add_for_ecg(m,p);
#else
			REAL p = fp32_mul_for_ecg(X, __hf_mem__(H,SI,I));
			REAL m = __dotp_mem__(H,SI);
			__dotp_mem__(H,SI) = fp32_add_for_ecg(m,p);
#endif
		}
	}
}
...
void computeMSE()
{
	...
	for (I=0; I<NSAMPLES; I++)
	{
		for (SI=0; SI<NSIGMAS; SI++)
		{
			...
			{
#ifdef USE_DOUBLE
				P  = fp64_add_for_ecg(P,  fp64_mul_for_ecg ( __dotp_mem__(H,SI) , __hf_mem__(H,SI,I)));
#else
				P  = fp32_add_for_ecg(P,  fp32_mul_for_ecg ( __dotp_mem__(H,SI) , __hf_mem__(H,SI,I)));
#endif

			}
#ifdef USE_DOUBLE
			REAL diff = fp64_sub_for_ecg (current_beat[I], P);
			err[SI] +=  fp64_mul_for_ecg(diff , diff);
#else
			REAL diff = fp32_sub_for_ecg (current_beat[I], P);
