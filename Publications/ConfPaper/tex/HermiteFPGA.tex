...
The hardware generated out of the algorithmic specification describe
above was mapped to an ML605 FPGA card from Xilinx.  The RIFFA 
software drivers and interface infrastructure was used to communicate
between the host and the FPGA card.  The round-trip delay is
the time interval between the beginning of transmission of beat-data
from the host to the hardware and the beginning of reception of best fit
coefficients from the hardware.
\begin{verbatim}
Four-way-unrolled latency = 0.43/0.47 ms (min/max).
\end{verbatim}
The estimated power dissipation is 10W (TBD).  The hardware utilization
is less than $30\%$ of the FPGA resource (TBD).
